/*
  ...
  1. Content
    - Add test user with demo credentials
    - Add test profile data
    - Add points history
    - Add rewards and user rewards
...
SELECT 
  ...
  id,
  CASE 
    WHEN required_level = 5 THEN 'Redeemed'
    WHEN required_level IN (10, 15) THEN 'Available'
    ELSE 'Locked'
  END,
  CASE 
...
SELECT 
  theme_data.name,
  theme_data.start_level,
  theme_data.end_level,
...
) AS theme_data(name, start_level, end_level, background_image)
WHERE NOT EXISTS (
  SELECT 1 FROM themes 
  WHERE (start_level, end_level) = (theme_data.start_level, theme_data.end_level)
);